...
object Requests {
  ...
  case class UpdateEdgeReq(targetNodeId: String, edgeType: String, direction: String, properties: EdgeProperties = Map.empty)
  ...
  case class GraphQueryReq(queries: List[QueryReq], nodeType: NodeType, nodeProperties: NodeProperties = Map.empty)
}
