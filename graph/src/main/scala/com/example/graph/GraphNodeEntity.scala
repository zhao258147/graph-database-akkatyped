...
object GraphNodeEntity {
  ...
  @JsonSubTypes(
    Array(
      ...
      new JsonSubTypes.Type(value = classOf[RemoveEdge], name = "RemoveEdge"),
      new JsonSubTypes.Type(value = classOf[EdgeQuery], name = "EdgeQuery"),
      new JsonSubTypes.Type(value = classOf[UpdateEdge], name = "UpdateEdge")))
  ...
  case class UpdateNodeCommand(properties: NodeProperties, replyTo: ActorRef[GraphNodeCommandReply]) extends GraphNodeCommand[GraphNodeCommandReply]
  case class RemoveEdge(targetNodeId: TargetNodeId, edgeType: EdgeType, replyTo: ActorRef[GraphNodeCommandReply]) extends GraphNodeCommand[GraphNodeCommandReply]
  case class UpdateEdge(targetNodeId: TargetNodeId, edgeType: EdgeType, direction: EdgeDirection, properties: EdgeProperties, replyTo: ActorRef[GraphNodeCommandReply]) extends GraphNodeCommand[GraphNodeCommandReply]

  case class NodeQuery(replyTo: ActorRef[GraphNodeCommandReply]) extends GraphNodeCommand[GraphNodeCommandReply]
  case class EdgeQuery(nodeType: Option[NodeType], nodeProperties: NodeProperties, edgeType: Option[EdgeType], edgeProperties: EdgeProperties, replyTo: ActorRef[GraphNodeCommandReply]) extends GraphNodeCommand[GraphNodeCommandReply]
  ...
  case class GraphNodeCommandSuccess(message: String = "") extends GraphNodeCommandReply
  case class GraphNodeCommandFailed(error: String) extends GraphNodeCommandReply
  ...
  case class GraphNodeUpdated(id: NodeId, nodeType: NodeType, columns: NodeProperties) extends GraphNodeEvent
  case class GraphNodeEdgeRemoved(targetNodeId: TargetNodeId, edgeType: EdgeType) extends  GraphNodeEvent
  case class GraphNodeEdgeUpdated(targetNodeId: TargetNodeId, edgeType: EdgeType, direction: EdgeDirection, properties: EdgeProperties) extends GraphNodeEvent
  ...
  case class CreatedGraphNodeState(nodeId: NodeId, nodeType: NodeType, properties: NodeProperties, edgesWithProperties: EdgesWithProperties, edges: Map[TargetNodeId, Seq[EdgeType]]) extends GraphNodeState
  ...
  (GraphNodeState, GraphNodeCommand[GraphNodeCommandReply]) => ReplyEffect[GraphNodeEvent, GraphNodeState] = {
    (state, command) =>
      logger.info(context.self.path.name)
      state match {
          ...
          command match {
            case create: CreateNodeCommand =>
              Effect
                ...
                .thenReply(create.replyTo)(_ => GraphNodeCommandSuccess())
            case cmd =>
              Effect.reply(cmd.replyTo)(GraphNodeCommandFailed("Node does not exist"))
          }
        case createdState: CreatedGraphNodeState =>
          command match {
            case create: CreateNodeCommand =>
              Effect.reply(create.replyTo)(GraphNodeCommandFailed("Node exists"))

            case update: UpdateNodeCommand =>
              Effect
                .persist(GraphNodeUpdated(createdState.nodeId, createdState.nodeType, update.properties))
                .thenReply(update.replyTo)(_ => GraphNodeCommandSuccess())
              ...
              existingEdge match {
                case Some(_) =>
                  Effect
                    ...
                    .thenReply(removeEdge.replyTo)(_ => GraphNodeCommandSuccess())
                case _ =>
                  Effect.reply(removeEdge.replyTo)(GraphNodeCommandFailed("Edge does not exist"))
              }

            case updateEdge: UpdateEdge =>
              Effect
                .persist(GraphNodeEdgeUpdated(updateEdge.targetNodeId, updateEdge.edgeType, updateEdge.direction, updateEdge.properties))
                .thenReply(updateEdge.replyTo)(_ => GraphNodeCommandSuccess())
    ...
    state match {
      ...
      case createdState: CreatedGraphNodeState =>
        event match {
          ...
          case edgeUpdated: GraphNodeEdgeUpdated =>
            val newEdge = Edge(edgeUpdated.edgeType, edgeUpdated.direction, edgeUpdated.properties)

            val newTargetEdges = createdState.edgesWithProperties.getOrElse(edgeUpdated.edgeType, Map.empty) + (edgeUpdated.targetNodeId -> newEdge)

            println(newTargetEdges)

            val newNodeEdges = edgeUpdated.edgeType +: createdState.edges.getOrElse(edgeUpdated.targetNodeId, List.empty)
            println(newNodeEdges)
            createdState.copy(
              edgesWithProperties = createdState.edgesWithProperties + (edgeUpdated.edgeType -> newTargetEdges),
              edges = createdState.edges + (edgeUpdated.targetNodeId -> newNodeEdges)
            )
        }
    }
  }
